...
export class AppComponent {
  ...
  loadFiles(callback){
    var options: any = {filters: [{name: 'Kingmaker Save File', extensions: ['zks'] }],
        properties: ['openFile','treatPackageAsDirectory', 'multiSelections', 'showHiddenFiles'],
        defaultPath: ''};
    if (window['process'].platform == 'win32')
      options.defaultPath = '%LOCALAPPDATA%\\..\\LocalLow\\Owlcat Games\\Pathfinder Kingmaker\\Saved Games\\'
    if (window['process'].platform == 'darwin')
    ...
    let parseFile = (filename) => {
      var data = this.fs.readFileSync(filename, 'binary');
      let zipfile = new this.zip(data);
      var partyString = zipfile.files['party.json'].asText();
      if (partyString.charCodeAt(0) == 65279)
        partyString = partyString.substring(1);
      let  party = this.resolveReferences(partyString);
      var playerString = zipfile.files['player.json'].asText();
      if (playerString.charCodeAt(0) == 65279)
        playerString = playerString.substring(1);
      let player = this.resolveReferences(playerString);
      return {player:player, party:party, filename: filename};

    };
     this.dialog.showOpenDialog(null,options, (fileNames) =>{
      if (fileNames === undefined) return;
      let results = [];
      for(let i = 0; i < 2 && i < fileNames.length; i++){
        results.push(parseFile(fileNames[i]));
      }
  ...
  compareFiles(){
    this.loadFiles((saveFiles) => {
      let filters = [
        ...
        /party\.m_EntityData\.\d+.SpawnPosition\..*/,
        /player\.GameTime/,
        /player\.RealTime/,
        /player\.m_CameraPos\..*/,
        /player\.Weather\..*/,
        ...
        /party\.m_EntityData\.\d+\.Descriptor\.ActivatableAbilities\..*\.Active*/,
        /party\.m_EntityData\.\d+\.Descriptor\.Abilities\..*\.Active*/,
        /party\.m_EntityData\.\d+\.Descriptor\.m_Inventory\.m_Items\.\d+\.HoldingSlot\.Owner\..*/,
        /party\.m_EntityData\.\d+\.Descriptor\.m_Inventory\.m_Items\.\d+\.Wielder\..*/,
        /.*\.Direction\.\$type/,
        /.*\.m_Point\.\$type/
      ]
      window['electron'].remote.getCurrentWebContents().openDevTools();
      let current, changed;
      if(saveFiles.length == 2){
        current = this.flatten(saveFiles[0]);
        changed = this.flatten(saveFiles[1]);
        console.log('Comparing ' + this.path.basename(saveFiles[0].filename) + ' & ' + this.path.basename(saveFiles[1].filename));
      } else {
        current = this.flatten({party:this.party, player:this.player});
        changed = this.flatten(saveFiles[0]);
        console.log('Comparing ' + this.path.basename(this.filename) + ' & ' + this.path.basename(saveFiles[0].filename));
      }
      let diff = {};
  ...
  recursiveBumpStatIds(parent, seen?) {
    ...
    for(let key in parent){
      ...
      if(seen.has(child) || typeof child !== 'object' || !child) continue;
